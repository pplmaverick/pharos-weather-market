...
// 補跑 Seoul 和 Bangkok（Taipei=4, Tokyo=5 已成功）
// 使用與 Round2Markets 相同的時間基準（腳本建立時間）
const now = Math.floor(Date.now() / 1000);
const TARGET_DATE = BigInt(now + 24 * 3_600);
const LOCK_TIME   = BigInt(now + 18 * 3_600);

const CITIES = [
  { name: "Seoul",   buckets: [24n, 27n, 30n, 33n] },
  { name: "Bangkok", buckets: [29n, 32n, 35n, 38n] },
];
...
async function main() {
  ...
  for (let i = 0; i < CITIES.length; i++) {
    const { name, buckets } = CITIES[i];

    if (i > 0) {
      console.log("    等待 5 秒避免 RPC 限速...");
      await sleep(5000);
    }
