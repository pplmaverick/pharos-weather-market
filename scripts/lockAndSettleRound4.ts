/**
 * Round 4 市場結算：lockMarket() → AdminOracle.submitResult()
 * 城市：New York(13) Mumbai(14) Cairo(15) Sao Paulo(16)
 * 溫度：2026-09-08 06:00 UTC, Open-Meteo Archive API, 整數攝氏度，無縮放
 */
import {
  createWalletClient,
  createPublicClient,
...
const MARKETS = [
  { id: 13, city: "New York",  temp: 18n },
  { id: 14, city: "Mumbai",    temp: 28n },
...
async function main() {
  ...
  console.log("=== Round 4 lockMarket() + submitResult() ===\n");
  ...
  for (let i = 0; i < MARKETS.length; i++) {
    ...
    try {
      ...
      if (receipt.status !== "success") {
        throw new Error(`lockMarket reverted for market ${m.id}`);
      }
    } catch (e: unknown) {
  ...
  for (let i = 0; i < MARKETS.length; i++) {
    ...
    try {
      ...
      console.log(`  ✓ status: ${receipt.status}  block: ${receipt.blockNumber}\n`);
      if (receipt.status !== "success") {
        throw new Error(`submitResult reverted for market ${m.id}`);
      }
    } catch (e: unknown) {
      const err = e as Error & { shortMessage?: string; details?: string };
      console.error(`  ✗ 失敗: ${err.shortMessage ?? err.message}`);
